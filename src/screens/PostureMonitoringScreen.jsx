...
import { StyleSheet, SafeAreaView, StatusBar, Alert, Linking } from "react-native"
...
const PostureMonitoringScreen = ({ navigation }) => {
  ...
  const { isMonitoring, sessionTime, postureStatus, postureScore, alertsCount, handleStartStop, resetSession } =
    usePostureMonitoring()

  const toggleCameraPosition = useCallback(() => {
    setCameraPosition((prevPosition) => (prevPosition === "front" ? "back" : "front"))
  }, [])
  ...
  useEffect(() => {
    const setupCameraAndPermissions = async () => {
      ...
      if (devices.length > 0 && hasPermission) {
        let selectedDevice = devices.find((device) => device.position === cameraPosition)

        if (!selectedDevice) {
          console.warn(
            `${cameraPosition} camera not found, trying ${cameraPosition === "front" ? "back" : "front"} camera.`,
          )
          selectedDevice = devices.find((device) => device.position === (cameraPosition === "front" ? "back" : "front"))
        }
        ...
        if (selectedDevice.id !== chosenCameraDevice?.id) {
          console.log(`Selected Camera Device: ${selectedDevice.position} (${selectedDevice.id})`)
        ...
        console.log("Permissions granted, but camera devices are still loading or not available.")
        setIsCameraActive(false)
        setChosenCameraDevice(null)
      }
    }
  ...
  }, [hasPermission, requestPermission, devices, navigation, chosenCameraDevice, cameraPosition])
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <CameraView ref={cameraRef} device={chosenCameraDevice} isActive={isCameraActive} />
